...
print(f"Set, {dataset}, Parameters: epochs-{epochs} | batch-{batch_size} | lr-{CLR_lr} | temp-{temperature} | class lr-{Class_lr}")
...
CLR_train_loader = DataLoader(CLR_train_data, batch_size=batch_size, shuffle=True, num_workers=0, pin_memory=True,
                              drop_last=True)
...
model = CLR(1, feature_dim).to(device)
